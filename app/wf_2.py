...
OUTPUT_THEME = os.path.join(BASE_DIR, 'output')
...
def convert_webflow_to_wp():
    theme_name = request.form.get('theme_name', 'converted-theme')
    webflow_zip = request.files.get('webflow_zip')

    if not webflow_zip:
        return jsonify({"error": "Missing required field: webflow_zip"}), 400

    theme_name = secure_filename(theme_name)
    temp_base = tempfile.mkdtemp()
    temp_dir = os.path.join(temp_base, 'unzipped')
    output_dir = os.path.join(temp_base, theme_name)
    os.makedirs(temp_dir, exist_ok=True)
    os.makedirs(output_dir, exist_ok=True)

    # Save and unzip Webflow ZIP
    ...
    # Copy starter theme
    shutil.copytree(STARTER_THEME, output_dir, dirs_exist_ok=True)
    ...
    theme_name = theme_name if theme_name else 'converted-theme'
