...
from services.priorities_integration import ClickUpPrioritiesIntegration
from services.google_calendar_service import GoogleCalendarService
from services.calendar_tasks_service import CalendarTasksService
import json
import requests
...
import time
from services.clickup_projects_integration import ClickUpProjectsIntegration
...
def get_task_details(task_id):
    ...
    try:
        ...
        clickup_integration = ClickUpPulseIntegration(
            api_token='pk_126127973_W2I64C72GTEQA77GMEV0YWG0TJR3GT0J',
            space_id=None
        )
        
        # Fetch task details from ClickUp API
        task_details = clickup_integration.get_task_details(task_id)
...
@app.route('/api/project-details/<project_id>')
def get_project_details(project_id):
    ...
    try:
        ...
        project_details = clickup_integration.get_project_details(project_id)
...
def get_pulse_data():
    try:
        ...
        hf_token = os.getenv('HUGGINGFACE_API_TOKEN', 'your_huggingface_token_here')  # Get from environment
        
        # NEW: Cache checking parameters
        check_cache_only = request.args.get('check_cache_only', 'false').lower() == 'true'
        check_today_cache = request.args.get('check_today_cache', 'false').lower() == 'true'
        ...
        if debug_mode:
            ...
            print(f"DEBUG: Check cache only: {check_cache_only}")
            print(f"DEBUG: Check today cache: {check_today_cache}")
            print(f"DEBUG: Force refresh: {force_refresh}")
            print(f"DEBUG: Using updated code with force_refresh fix")
        ...
        # NEW: Smart caching logic based on parameters
        if check_cache_only:
            # Only check for recent cache (< 3 hours old)
            print("📁 Check cache only mode - looking for recent cache...")
            pulse_data = clickup_integration.get_recent_cached_data_only(
        ...
        elif check_today_cache:
            # Check for any cache from today (even if > 3 hours old)
            print("📁 Check today cache mode - looking for today's cache...")
            pulse_data = clickup_integration.get_today_cached_data_only(
                target_date=target_date,
                debug=debug_mode,
                status_filters=status_filters,
                member_filters=member_filters,
            ...
            print("🔄 Normal mode - using existing pulse analytics logic...")
            print(f"🔄 Calling generate_pulse_analytics_with_ai with force_refresh={force_refresh}")
            pulse_data = clickup_integration.generate_pulse_analytics_with_ai(
                target_date=target_date,
                debug=debug_mode,
                hf_token=hf_token,
                status_filters=status_filters,
            ...
            print(f"🔄 Pulse data result: data_source={pulse_data.get('data_source', 'unknown') if pulse_data else 'None'}")
...
@app.route('/api/priorities-data')
def get_priorities_data():
    """Get priorities data with smart caching"""
    try:
        from services.priorities_integration import ClickUpPrioritiesIntegration
        ...
        print(f"🎯 Priorities API called - force_refresh={force_refresh}, check_cache_only={check_cache_only}")
        
        # Initialize priorities integration
        priorities_integration = ClickUpPrioritiesIntegration()
        
        if check_cache_only:
            # Only check for cached data
            print("📁 Checking for cached priorities data...")
            cached_data = priorities_integration._get_cached_priorities_data(selected_date)
            if cached_data:
                print("✅ Returning cached priorities data")
                return jsonify(cached_data)
            else:
                print("❌ No cached priorities data found")
                return jsonify({"error": "No cached priorities data available"}), 404
        
        # Generate priorities data
        priorities_data = priorities_integration.generate_priorities_data(
            force_refresh=force_refresh,
            selected_date=selected_date
        )
        
        if priorities_data:
            print("✅ Priorities data generated successfully")
            return jsonify(priorities_data)
        else:
            print("❌ Failed to generate priorities data")
            return jsonify({"error": "Failed to generate priorities data"}), 500
            
    except Exception as e:
        print(f"❌ Error in priorities API: {e}")
        return jsonify({"error": f"Internal server error: {str(e)}"}), 500

@app.route('/generate_assessment')
def generate_assessment():
    """Generate comprehensive team assessment using OpenAI"""
    try:
        # Get the latest pulse data to generate assessment from
        target_date = request.args.get('date')
        debug_mode = request.args.get('debug', 'false').lower() == 'true'
        hf_token = os.getenv('HUGGINGFACE_API_TOKEN', 'your_huggingface_token_here')
        ...
        space_filters = request.args.getlist('space_filter')
        if not space_filters:
            space_filters = ["90132462540", "90134767504", "90138214659", "90138873649"]
        ...
        if not pulse_data:
            return jsonify({
                "success": False,
                "error": "No pulse data available for assessment"
            }), 404
        
        # Generate comprehensive assessment
        assessment = clickup_integration.generate_comprehensive_assessment(pulse_data)
...
def get_projects_data():
    """
    Get AI-powered project analytics data from ClickUp
    Implements 3-hour caching strategy with force refresh option
    """
    try:
        force_refresh = request.args.get('force_refresh', 'false').lower() == 'true'
        debug_mode = request.args.get('debug', 'false').lower() == 'true'
        ...
        if debug_mode:
            print(f"DEBUG: Projects API called with force_refresh={force_refresh}")
        
        # Initialize ProjectsService with ClickUp token
        projects_service = ProjectsService(
            clickup_token='pk_126127973_W2I64C72GTEQA77GMEV0YWG0TJR3GT0J',
            base_dir=BASE_DIR
        )
        
        # Get projects data (with caching logic)
        projects_data = projects_service.get_projects_data(force_refresh=force_refresh)
        
        if projects_data:
            # Add debug information
            debug_info = {
                "raw_projects_count": len(projects_data.get("projects", [])),
                "total_projects": projects_data.get("total_projects", 0),
                "filtered_projects": projects_data.get("filtered_projects", 0),
                "excluded_projects": projects_data.get("excluded_projects", 0),
                "project_names": [p.get("name", "Unknown") for p in projects_data.get("projects", [])]
            }
...
# Add this new endpoint after the existing @app.route('/api/projects-data') endpoint

@app.route('/api/projects-lists-data')
def get_projects_lists_data():
    """
    New API endpoint for ClickUp Lists (Projects) Integration
    Uses the new ClickUpProjectsIntegration service for enhanced project analytics
    """
    try:
        force_refresh = request.args.get('force_refresh', 'false').lower() == 'true'
        debug_mode = request.args.get('debug', 'false').lower() == 'true'
        ...
        if debug_mode:
            print(f"DEBUG: New Projects Lists API called with force_refresh={force_refresh}")
        
        # Initialize new ClickUpProjectsIntegration service
        projects_integration = ClickUpProjectsIntegration(
            api_token='pk_126127973_W2I64C72GTEQA77GMEV0YWG0TJR3GT0J'
        )
        
        # Get projects data using the new service
        projects_data = projects_integration.get_projects_data(force_refresh=force_refresh)
        ...
        if projects_data:
            return jsonify({
                ...
                "source": "new_clickup_projects_integration"
            })
        else:
            return jsonify({
                "success": False,
                "error": "Failed to fetch projects data from new integration"
            }), 500
            
    except Exception as e:
        print(f"Error fetching projects data from new integration: {e}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def process_html_files_with_seo(temp_dir, output_theme, seo_enabled=False, 
    ...
    if seo_enabled:
        try:
            ...
            print(f"SEO Service initialization failed: {e}")
            seo_service = None
        ...
        try:
            ...
            if seo_enabled and seo_service:
                try:
                    ...
                    print(f"SEO optimization failed for {filename}: {e}")
...
def assessment_report():
    """Generate professional assessment report page from cached pulse data"""
    try:
        import glob
        import os
        from datetime import datetime
        
        # Get the latest pulse data file from the output/pulse directory
        pulse_dir = os.path.join(BASE_DIR, 'output', 'pulse')
        pulse_files = glob.glob(os.path.join(pulse_dir, "pulse_*.json"))
        
        if not pulse_files:
            return "No pulse data available. Please run a pulse scan first.", 404
        
        # Sort by modification time and get the latest
        pulse_files.sort(key=os.path.getmtime, reverse=True)
        latest_file = pulse_files[0]
        
        print(f"📁 Using cached pulse data from: {os.path.basename(latest_file)}")
        
        # Read the cached pulse data
        with open(latest_file, 'r') as f:
            pulse_data = json.load(f)
        
        print(f"📊 Pulse data keys: {list(pulse_data.keys())}")
        print(f"👥 Member workloads: {len(pulse_data.get('member_workloads', {}))} members")
        print(f"📈 Overview stats: {pulse_data.get('overview_stats', {}).get('total_members', 'N/A')} members")
        print(f"📊 Workload distribution: {pulse_data.get('overview_stats', {}).get('workload_distribution', {})}")
        
        # Map the data structure to match what the template expects
        # The template expects certain field names that might be different in the JSON
        member_workloads = pulse_data.get('member_workloads', {})
        
        # Convert member_workloads from object to array format for the template
        member_workloads_array = []
        for member_name, member_data in member_workloads.items():
            member_workloads_array.append({
                'member_name': member_name,
                'workload_score': member_data.get('workload_score', 0),
                'active_tasks': member_data.get('active_tasks', 0),
                'projects_count': member_data.get('projects_count', 0),
                'urgent_tasks': member_data.get('urgent_tasks', 0),
                'high_priority_tasks': member_data.get('high_priority_tasks', 0)
            })
        
        # Calculate project performance metrics if they don't exist
        project_analytics = pulse_data.get('project_analytics', {})
        project_performance = {
            'projects_on_track': 0,
            'projects_delayed': 0,
            'avg_delay_days': 0,
            'total_projects_analyzed': len(project_analytics)
        }
        
        # Add weight field to projects for chart compatibility
        for project_id, project in project_analytics.items():
            if project:
                # Estimate weight based on task count and priority
                task_count = project.get('active_tasks', 0)
                priority = project.get('priority', 'normal')
                
                if priority == 'urgent':
                    project['weight'] = max(50, task_count * 15)
                elif priority == 'high':
                    project['weight'] = max(30, task_count * 12)
                elif priority == 'medium':
                    project['weight'] = max(20, task_count * 10)
                else:
                    project['weight'] = max(10, task_count * 8)
                
                # Add date_created for development duration calculation
                project['date_created'] = pulse_data.get('last_updated', '')
        
        mapped_data = {
            'member_workloads': member_workloads_array,  # Now it's an array
            'overview_stats': pulse_data.get('overview_stats', {}),
            'workload_distribution': pulse_data.get('overview_stats', {}).get('workload_distribution', {}),
            'project_analytics': project_analytics,  # Now with weight and date_created
            'project_performance': project_performance,
            'total_members': pulse_data.get('overview_stats', {}).get('total_members', 0),
            'total_tasks': pulse_data.get('overview_stats', {}).get('total_active_tasks', 0),
            'average_workload_score': pulse_data.get('overview_stats', {}).get('avg_workload_score', 0),
            'health_score': pulse_data.get('overview_stats', {}).get('health_score', 0),
            'recommendations': pulse_data.get('recommendations', []),
            'intelligent_summary': pulse_data.get('intelligent_summary', {}),
            'last_updated': pulse_data.get('last_updated', ''),
            'data_source': pulse_data.get('data_source', 'cached_data')
        }
        
        print(f"📊 Mapped data keys: {list(mapped_data.keys())}")
        print(f"👥 Mapped member workloads: {len(mapped_data['member_workloads'])} members")
        print(f"📈 Mapped workload distribution: {mapped_data['workload_distribution']}")
        print(f"📊 Mapped project analytics: {len(mapped_data['project_analytics'])} projects")
        
        # Initialize ClickUp integration service for assessment generation only
        clickup_integration = ClickUpPulseIntegration(
            api_token='pk_126127973_W2I64C72GTEQA77GMEV0YWG0TJR3GT0J',
            space_id=None
        )
        
        # Generate comprehensive assessment from cached data
        assessment = clickup_integration.generate_comprehensive_assessment(pulse_data)
        
        if assessment:
            # Add cache info to the assessment
            assessment['cache_info'] = {
                'source_file': os.path.basename(latest_file),
                'generated_date': datetime.fromtimestamp(os.path.getmtime(latest_file)).strftime('%Y-%m-%d %H:%M:%S'),
                'is_cached': True
            }
            
            return render_template('assessment_report.html', 
                                assessment=assessment, 
                                pulse_data=mapped_data,
                                filters={})
        else:
            return "Failed to generate assessment from cached data", 500
            
    except Exception as e:
        return f"Internal server error: {str(e)}", 500


@app.route('/debug_assessment_data')
def debug_assessment_data():
    """Debug route to check assessment data structure"""
    try:
        import glob
        import os
        from datetime import datetime
        
        # Get the latest pulse data file
        pulse_dir = os.path.join(BASE_DIR, 'output', 'pulse')
        pulse_files = glob.glob(os.path.join(pulse_dir, "pulse_*.json"))
        
        if not pulse_files:
            return jsonify({"error": "No pulse files found"})
        ...
        # Return the data structure for debugging
        return jsonify({
            "source_file": os.path.basename(latest_file),
            "data_keys": list(pulse_data.keys()),
            "member_workloads_count": len(pulse_data.get('member_workloads', {})),
            "overview_stats": pulse_data.get('overview_stats', {}),
            "project_analytics_count": len(pulse_data.get('project_analytics', {})),
            "has_recommendations": bool(pulse_data.get('recommendations')),
            "has_intelligent_summary": bool(pulse_data.get('intelligent_summary'))
        })
        
    except Exception as e:
        return jsonify({"error": f"Debug error: {str(e)}"})

@app.route('/api/projects-analytics')
def get_projects_analytics():
    """
    Enhanced API endpoint for ClickUp Projects Analytics Integration
    Uses the new ClickUpProjectsIntegration service for AI-powered project analysis
    """
    try:
        force_refresh = request.args.get('force_refresh', 'false').lower() == 'true'
        debug_mode = request.args.get('debug', 'false').lower() == 'true'
        ...
        if debug_mode:
            print(f"DEBUG: Enhanced Projects Analytics API called with force_refresh={force_refresh}")
        
        # Initialize enhanced ClickUpProjectsIntegration service
        projects_integration = ClickUpProjectsIntegration(
            api_token='pk_126127973_W2I64C72GTEQA77GMEV0YWG0TJR3GT0J'
        )
        
        # Get enhanced projects data with AI assessment
        projects_data = projects_integration.get_projects_data(force_refresh=force_refresh)
        ...
        if projects_data:
            return jsonify({
                ...
                "cache_info": {
                    ...
                    "filtered_projects": projects_data.get("filtered_projects", 0),
                    "excluded_projects": projects_data.get("excluded_projects", 0),
                    "teams_count": projects_data.get("teams_count", 0),
                    "spaces_count": projects_data.get("spaces_count", 0)
                },
                "source": "enhanced_clickup_projects_integration"
            })
        else:
            return jsonify({
                "success": False,
                "error": "Failed to fetch enhanced projects data"
            }), 500
            
    except Exception as e:
        print(f"Error fetching enhanced projects data: {e}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 500
...
def get_calendar_events():
    ...
    try:
        ...
        # Get calendar events
        calendar_data = calendar_service.get_calendar_events(days=days, use_cache=use_cache)
...
def get_calendar_tasks():
    """Get ClickUp tasks for calendar display"""
    try:
        force_refresh = request.args.get('force_refresh', 'false').lower() == 'true'
        check_cache_only = request.args.get('check_cache_only', 'false').lower() == 'true'
        check_today_cache = request.args.get('check_today_cache', 'false').lower() == 'true'
        
        calendar_tasks_service = CalendarTasksService()
        tasks_data = calendar_tasks_service.get_calendar_tasks(
            force_refresh=force_refresh,
            check_cache_only=check_cache_only,
            check_today_cache=check_today_cache
        )
...
@app.route('/api/calendar-test-connection')
...
def get_calendar_config():
    """Get current calendar configuration (without sensitive data)"""
    try:
        # Initialize Google Calendar service
        calendar_service = GoogleCalendarService()
        ...
        config = {
            ...
            'has_api_key': bool(calendar_service.api_key),
            'has_oauth_credentials': bool(calendar_service.client_id and calendar_service.client_secret),
            'has_refresh_token': bool(calendar_service.refresh_token),
            'auth_methods_available': []
        }
        
        if calendar_service.api_key:
            config['auth_methods_available'].append('api_key')
        if calendar_service.client_id and calendar_service.client_secret and calendar_service.refresh_token:
            config['auth_methods_available'].append('oauth')
...
@app.route('/api/calendar/oauth-url')
def get_calendar_oauth_url():
    """Generate OAuth URL for Google Calendar"""
    try:
        client_id = os.getenv('GOOGLE_CALENDAR_CLIENT_ID', '')
        redirect_uri = 'http://localhost:5022/api/calendar/oauth-callback'
    ...
    try:
        ...
        if refresh_token:
            return jsonify({
                ...
                'message': 'OAuth successful! Copy the refresh_token to your .env file'
            })
        else:
            return jsonify({
                'success': False,
